...
treateff_hdl <- 2 * 0.02586 * 10 # TRANSFORMATION *10 FOR MODEL INPUT # Treatment effect on HDL-cholesterol (absolute effect in mmol/l)
...
treateff_bmi <- -3 # Treatment effect on BMI (in absolute points)
treateff_sbp <- -5/ 10 # TRANSFORMATION /10 FOR MODEL INPUT# Treatment effect on SBP (in absolute mmHg)


# Tx effects are vectors: the current assumption is that the same start, end and decline is assumed for all effect modifiers
treatment_effect_HbA1c_input <- c(treateff_hba1c, 1, 15, 2)
treatment_effect_HDL_input <- c(treateff_hdl, 1, 15, 2)
treatment_effect_LDL_input <- c(treateff_ldl, 1, 15, 2)
treatment_effect_BMI_input <- c(treateff_bmi, 1, 20, 2)
treatment_effect_SBP_input <- c(treateff_sbp, 1, 10, 2)
...
print(paste(end, 'End of LookAHEAD analysis'))
print('Duration of LookAHEAD analysis')
print(end-init)
...
save(sim.vars,
     ...
     file = paste0('output/', country.id, '/LookAHEAD_run_', run_id, '.RData'))
