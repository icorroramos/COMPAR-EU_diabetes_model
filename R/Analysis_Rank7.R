...
treateff_hba1c <- -0.168 # Treatment effect on HbA1c (in absolute %-points HbA1c)
treateff_hdl   <- -0.2707 * 0.02586 * 10 # TRANSFORMATION *10 FOR MODEL INPUT # Treatment effect on HDL-cholesterol (absolute effect in mmol/l)
treateff_ldl   <- -2.9532 * 0.02586 * 10 # TRANSFORMATION *10 FOR MODEL INPUT# Treatment effect on LDL-cholesterol (absolute effect in mmol/l)
treateff_bmi <- -0.8234 # Treatment effect on BMI (in absolute points)
treateff_sbp <- -4.2854 / 10 # TRANSFORMATION /10 FOR MODEL INPUT# Treatment effect on SBP (in absolute mmHg)
...
print(paste(end, 'End of Rank 7 analysis'))
print('Duration of Rank 7 analysis')
print(end-init)
...
save(sim.vars,
     sim.results.female,
     sim.results.male,
     file = paste0('output/', country.id, '/Rank7_basecase_', npats_input, '-pats_seed-', seed_input, '.RData'))
