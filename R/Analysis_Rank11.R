...
treateff_hdl   <- 3.5797 * 0.02586 * 10 # TRANSFORMATION *10 FOR MODEL INPUT # Treatment effect on HDL-cholesterol (absolute effect in mmol/l)
treateff_ldl   <- -16.632 * 0.02586 * 10 # TRANSFORMATION *10 FOR MODEL INPUT# Treatment effect on LDL-cholesterol (absolute effect in mmol/l)
...
print(paste(end, 'End of Rank 11 analysis'))
print('Duration of Rank 11 analysis')
print(end-init)
...
save(sim.vars,
     sim.results.female,
     sim.results.male,
     file = paste0('output/', country.id, '/Rank11_basecase_', npats_input, '-pats_seed-', seed_input, '.RData'))
